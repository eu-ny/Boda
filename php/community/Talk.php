...
if ($result) {
    ...
    if ($count > 0) {
        for ($i = 1; $i <= $count; $i++) {
            ...
            echo "<tr id='" .
                $info['myTalkID'] .
                "'><td class='comment comment_1'><div class='profile'></div><div class='contents'><div class='contents__top'>";
            echo "<p class='name'><span class='ir'>작성자</span><span>" .
                $info['youNickName'] .
                '</span></p>';
            echo "<p class='date'><span class='ir'>작성일</span><span>| " .
                date('Y-m-d H:i', $info['TalkregTime']) .
                '</span></p>';
            if ($_SESSION['myMemberID'] == $info['myMemberID']) {
                echo "<a href='#' class='Talkmodify'>| 수정</a><a href='#' class='Talkdelete'>| 삭제</a></div>";
            }
            echo "<div class='contents__bottom'><span>" .
                $info['TalkContents'] .
                '</span></div></div>';
            echo '</td></tr>';
        }
